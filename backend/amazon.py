...
class AmazonConfig:
    ...
    def __init__(self) -> None:
        self.amazon_urls = ["https://amazon.com", "https://amazon.co.uk"]
...
class AmazonLocators:
    ...
    PRODUCT_PRICE = get_locator("div", "id", "corePriceDisplay_desktop_feature_div")
...
class AmazonScraper:
    ...
    def get_page_source(self, url):
        ''' Get page source '''
        with self.lock:
            url_bd = "https://api.brightdata.com/request"

            payload = {
                "zone": "streamlit_unlocker",
                "url": url,
                "format": "raw"
            }
            headers = {
                "Authorization": f"Bearer {st.secrets.brightdata['key']}",
                "Content-Type": "application/json"
            }

            response = requests.request("POST", url_bd, json=payload, headers=headers)
    ...
    def _get_price_and_currency_from_string(self, price_string: str) -> tuple[str, str]:
        ...
        if (gbp_curr := "\xA3") in price_string:
            price = price_string.split(gbp_curr)[1].split()[0]
            currency = "GBP"

        elif (usd_curr := "\u0024") in price_string:
            price = price_string.split(usd_curr)[1].split()[0]
            currency = "USD"
        else:
            currency = ""
            price = price_string
